...
BEGIN
    ...
    PERFORM anon_table('res_user', 'name, login', 'email', 'id: not in :(select id from res_user where login = ''admin'');id: not in :(select "user" from "res_user-res_group" where "group" in (select id from res_group where name=''Coog Do Not Anonymize''))');
    ...
    PERFORM anon_table('ir_attachment', 'name, description', 'data');
    ...
    PERFORM anon_table('account_invoice_tax', 'description');
    PERFORM anon_table('account_move', 'description');
    PERFORM anon_table('account_move_line', 'description');
    PERFORM anon_table('account_payment_sepa_mandate', 'identification');
    PERFORM anon_table('account_statement_origin', 'description');
    PERFORM anon_table('bank_account_number', 'number, number_compact');
    PERFORM anon_table('document_signature', 'logs');
    PERFORM anon_table('dsn_message', 'errors');
    PERFORM anon_table('ir_email', 'body, recipients, recipients_hidden, recipients_secondary, subject');
